...
% * Move section 'Dual Localization' to its own
%   section after 'Extended Source Detection Threshold'
%   and rename it 'Testing Against Source Confusion'.
%   The section is significantly expanded with a new
%   monte carlo study fitting extended sources as
%   two point-like sources, and vice versa
%
% * Major rewrite of ``Extended Source Search Method'' section.
%   This section now more correctly explains how the
%   analysis search was done as well as cautions the
%   reader about what caveats may apply to the analysis.

% * Major rewrite of the sections on the new extended sources
%   to better describe why various point sources were removed
%   and why some were kept using multiwavelenth information.

% * Move AGN validation section earlier in the text
%   so that it is the first section analyzing actual LAT 
% * Move section on reanalyzing 2FGL sources
%   to earlier in the paper to be right after the AGN
%   validation.
% * Move earlier section discussing systematic errors
% * better description of how two of the 'new extended sources'
%   already have dedicated publications

% * Remake 2FGL J1632.4-4753c smoothed counts map plot to show
%   before + after background subtraction
%
% * Reordered Gamma-Cygni section to first discuss the pulsar.
%
% * For HESS J1837, smooth image by a 0.15 degree Gaussain
%   instead of a 0.1 degree gausian
%   kernel to surpress some of the noise which is likely spurious.
%   Also, add in some lower level contour from HESS data
%   to better show how wide the HESS emission is.
%
% * Rewrote the 'preview of what's to come section
%   in the introduction to have the right order.
%
% * Cleaned up the Comparing source size section.
% * Better description of converting TSext into sigma.
% * better warning about the limits of the alternate diffuse model.
% * Fixed figure order.
% * made tickmarks white in plots.
% * replace all occurrences of ts_inc with ts_2pts
% * changed zoom of IC443 res tsmap to be easier to view.
% * Added LAT collaboration citation of W30.
%
% * Added acknowlegements section to end of the paper
% * Move HESS SEDs to earlier in paper to fix order.
% * Fix some occurances of chi^2 to correctly be chi^2_1
% * Better description of how the alternate diffuse model
%   is different from galprop
% * Add galprop citation
% * Add AGILE citation Gamma Cygni Pulsar citation.
...
spatially extended sources reported in 2FGL and two other sources studied
...
\newtext{
Previous analysis of extended LAT sources were performed as dedicated
...
study the ability of the LAT to distinguish between spatially
extended sources and confusion of nearby point-like sources.
...
but as being point-like.  Two of these have been previously analyzed in
...
for a uniform surface brightness spatial model).
...
\newtext{
We considered two different models to use for the
...
we defined here the source size (0\fdg5) 
...
For this example, where $\rsixeight=0\fdg5$, 
$\sigma_\text{disk}=0.61\degree$ and 
$\sigma_\text{Gaussian}=0.33\degree$.

For smaller and fainter extended sources, the differences in the PDF
for different spatial models are lost in the noise and the LAT is not
sensitive to the detailed spatial structure of the sources.  Therefore,
in our search for extended sources we use only a radially-symmetric
uniform surface brightness as our spatial extension shape. We quote the
radius of the edge ($\sigma$) as the size of the source.
}
...
\newtext{
The threshold is high for small extensions because the
source becomes
small compared to the PSF. The threshold increases
...
We assessed the power of the $\tsext>\tsinc$ test using a Monte Carlo
study.  We simulated one spatially extended sources and fit it as both
...
how well comparing \tsext to \tsinc is at rejecting the possibility of
source confusion as well as how likely it is to incorrectly reject an
extended source as being extended.  All sources were simulated using
...
when looking in the 1 \gev and 100 \gev energy range
...
flux of the spatially extended source but the same spectral index.
...
The distributions show that for simulated
...
than the one extended source (one extension parameter)
and can therefore easy model
statistical fluctuations.
This effect is most pronounced when using energies from 1
\gev to 100 \gev where the PSF is even larger.  
On the other hand,
...
This is because the one additional extension degree of freedom does
now as easily fit fit statistical fluctuations.

From this, we conclude that the $\tsext>\tsinc$ test is reasonably
...
in figure~\ref{confusion_plot}a and figure~\ref{confusion_plot}a when
...
calculated by adding in quadrature the statistical error on the spectral
index.  This plot shows that there can be a large difference in the
...
instead of the emission from two confused point-like sources.
...
that we have a statistical test which can rigorously find sources for
which the spatial emission is not well described by the PSF.  But for
the extended sources we are searching for, this extension test does
...
$\tsext>\tsinc$ is a reasonably powerful test to avoid cases of simple
confusion with two point-like sources. But it could always be the
case that an extended source candidate is actually the superposition
of multiple point-like or extended sources which could be resolved
with deeper observations of the region.  Of course, there is nothing
about this conclusion unique to LAT analysis.  Its just that the 
...
When possible, multiwavelength information should be used to help decide
what is qualitatively the best sky model.
...
of background sources and we refit the spectral parameters of sources
...
each of these sources required a dedicated analysis and these sources
...
method and consistently assuming a radially-symmetric uniform surface
brightness for their spatial model.  On the other hand, for some of
these sources a uniform disk spatial model does not well describe the
observed extended emission and so the dedicated LAT publications give
a qualitatively better model of the source.
...
\newtext{
For the LAT detected extended sources presented in this paper,
we developed two criteria for estimating the systematic error
on the extension of the source.
...
maps it is unable to asses systematics due to misidentified of local
...
Now that we have demonstrated that we can analyze sources
previously known to be extended at \gev energies
(Section~\ref{validate_known})
and that we understand the statistical issues associated with
this analysis (Section~\ref{monte_carlo_validation} and~\ref{test_2lac_sources}),
we applied this method to search for new spatially extended \gev sources.

Ideally, we would have applied a completely blind search where we simply
fit the extension of each catalog source in the presence of all other 2FGL
sources to find a complete list of all the sources that are spatially
extended.  And as our test of AGN in Section~\ref{test_2lac_sources}
showed, at high galactic latitude where the source density is not as
great and the diffuse emission is less structured, this method works well.

This method is infeasible in the galactic plane where we
most likely expect to discover new spatially extended sources.  In the
galactic plane, the analysis becomes challenged by our imperfect model of
the diffuse emission and by issues related to source confusion.  
Finding sources on top of this emission
is difficult as was discussed in 1FGL
and 2FGL \citep{first_cat,second_cat} and we expect that finding
spatially extended sources be even more difficult.
The Monte
Carlo study on source confusion in Section~\ref{dual_localization_method}
showed that the overall likelihood would greatly increase when fitting
a spatially extended source as two point-like sources. So we expect that
most spatially extended LAT sources that are
currently modeled as point-like in 2FGL are surrounded by other 
point-like sources that representing
residual induced by not correctly modeling the sources' extension.
Furthermore, the position of other nearby sources in the region near
an extended source could be biased by not taking the extended
sources' extension into account.  Furthermore, 2FGL produced a
list of sources significant at energies above 100 \mev
whereas we are most sensitive to spatial extension at higher energies.
Therefore, we expect that when studying spatially extended sources
at higher energies our analysis would be complicated
by 2FGL sources not significant at higher energies and 2FGL sources
whose position was biased by the diffuse emission at lower energies.

To account for these issues,
we first produces a large list of
possibly extended sources employing a very liberal search criteria
and then refined the analysis of the promising candidates on a case by case basis.
Our strategy was to test all point-like 2FGL sources
for extension by assuming that they have a uniform radially-symmetric
uniform surface brightness
and that they have a power-law spectral model.
Although not all extended sources are
expected to have a shape very similar to a uniform surface brightness,
Section~\ref{compare_source_size} showed that for many spatially
extended sources the wide PSF of the LAT and limited statistics allows
for this to be a reasonable approximation.
Additionally,
our Monte Carlo studies (Section~\ref{monte_carlo_validation}
and~\ref{test_2lac_sources}) demonstrate that we can use this shape for a
statistically rigorous test. Because this spatial model only
adds one additional degree of freedom, this
test should be very sensitive to most spatially extended sources.
On the other hand, this spatial model could bias us against
finding extended sources which are not well described by uniform
emission such as certain shell-type supernova remnants.

Before testing for extension,
we automatically removed from the background model all other 2FGL
sources within 0\fdg5 of the source.  This value is somewhat arbitrary,
...
energies between 10 \gev and 100 \gev.  Although this approach tests the
same sources, it is complementary because the Galactic diffuse emission is
even less dominant above 10 \gev. Furthermore, source confusion becomes
less of a problem since most LAT sources are not significantly detected
at energies exclusively above 10 \gev. So we expected to be able to resolve
harder sources in more complicated regions by restricting our analysis
to higher energies. The $>10$ \gev analysis is especially beneficial
for regions near pulsars which are not significantly detected above
10 \gev. A similar procedure was used to detect HESS\,J1825$-$137 and
MSH\,15$-$52 with the LAT \citep{msh1552,fermi_hess_j1825}.
...
of residual \ts by adding a new source of spectral index 2 into the
region at each pixel and finding the increase in likelihood when fitting
its flux. Figure~\ref{res_tsmaps} shows a residual \ts map around the
most significantly extended source IC443 when it is modeled both as a
point-like sources and an extended source.  The residual \ts map indicates
...
It was very clear by visually inspecting many of these plots that in many
...
shows a diffuse-emission-subtracted smoothed counts map with the best
...
Because of these systematic issues, we only to selected candidates in
regions which seemed relatively simple or in regions where there was an
obvious spatially extended multiwavelength counterpart. Furthermore,
by assuming extended sources had a uniform spatial emission our
search was biased towards finding sources with this certain type of
morphology. Therefore, our search can not be expected to be complete
and it is likely that there are other spatially extended sources that
this method missed.
...
for residual induced by not taking the extension of the extended source
into account and sources include in 2FGL to represent soft point-like
...
multiwavelenth information about possibly extended source counterparts
...
Therefore, after selecting the list of sources, we had to iteratively
refit the position and spectrum of nearby background sources as well as
the position, extension, and spectrum of the analyzed spatially extended
sources until the overall fit had globally converged.  For each extended
source, we will describe these modifications.
...
We required also that $\tsext>16$ using the alternate diffuse
model presented in seciton~\ref{systematic_errors_on_extension}.
we then replaced the spatially extended source with two point-like
sources and refit their positions and spectrum and calculate \tsinc.
...
radially-symmetric uniform surface brightness model.
...
analysis of these sources to higher energies because of the issues
...
in Table~\ref{new_ext_srcs_table}.  The results of our investigation of
systematic errors are presented in Table~\ref{alt_diff_model_results}.
This section compares the fit extension with an extended source hypothesis to the
fit assuming the emission originates from two independent point-like
sources. For these extended extended sources, $\tsext>\tsinc$ suggesting
that the \gev emission does not originate from multiple physically
distinct sources.  Table~\ref{alt_diff_model_results} also includes the
...
this source.  There are two nearby catalog sources 2FGL\,J0823.4$-$4305
and 2FGL\,J0821.0$-$4254 which are also coincident with the SNR but that
do not separate into multiple point-like components.  We concluded that
these two sources were included in 2FGL to account for residual induced
by not taking the extension of this extended source into account and
remvoed them from our model of the sky.

With this model, 2FGL\,J0823.0$-$4246 was found to have an extension of
$0\fdg37\pm0\fdg03_\stat\pm0\fdg02_\sys$ with an extension significance
of $\tsext=46.3$ using photons with energies between 1 \gev and 100 \gev.
This source was fit to $(l,b)=(260\fdg32,-3\fdg28)$ and is also coincident
with 1FGL\,J0823.3$-$4248.  Figure~\ref{snr_seds} shows the spectrum of
this source.
...
match well its inferred size at \gev energies.  Puppis A's distance was
...
was fit to
$(l,b)=(353\fdg08, 16\fdg78)$ and is also coincident with 
1FGL\,J1628.6$-$2419c.  
...
photons with energies between 1 \gev and 100 \gev.  It is
...
and overlays H.E.S.S. \tev contours of SNR
...
of $\tsext=39.6$.  This source was fit to $(l,b)=(347\fdg25,-0\fdg54)$
and is also coincident with 1FGL\,J1711.7$-$3944c.  Figure
\ref{2FGL_J1712.4-3941} shows a counts map of this source.
}
...
candidate in the 10 \gev to 100 \gev energy range and is spatially
coincident with the SNR Vela Jr.  Figure~\ref{Vela_Jr} shows a counts
map of the source.  Overlaid on Figure~\ref{Vela_Jr} are H.E.S.S. \tev
contours of Vela Jr.  \citep{vela_jr_hess} and show that three
point-like catalog sources 2FGL\,J0853.5$-$4711, 2FGL\,J0848.5$-$4535,
and 2FGL\,J0855.4$-$4625 correlate with the multiwavelenth emission of
this SNR are were most likely included in 2FGL to account for residual
induced by not taking the extension of this extended source into account.
These sources were removed from our model of the sky.  In addition,
two further away sources 2FGL\,J0858.0$-$4815 and 2FGL\,J0901.7$-$4655
were removed from our model of the sky because they were not significant
above 10 \gev.

With this model of the sky, 2FGL\,J0851.7$-$4635 was found to have an
extension of $1\fdg13\pm0\fdg08_\stat\pm0\fdg05_\sys$ with an extension
significance of $\tsext=87.2$ using photons with energies between 10
\gev and 100 \gev.  This source was fit to $(l,b)=(266\fdg29,-1\fdg43)$.
While fitting the extension of 2FGL\,J0851.7$-$4635, we iteartively
...
position at high energies.  The LAT size matches well the \tev morphology
of Vela Jr.  A detailed papers by the LAT Collaboration on this SNR is
in preparation.
}
...
range. They are less than $1\degree$ away from eachother and needed
...
of the source and overlays the \tev contours of HESS\,J1616$-$508 and
HESS\,J1614$-$518 \citep{hess_plane_survey}.  The figure shows that the
source 2FGL\,J1614.9$-$5212 is very close to 2FGL\,J1615.2$-$5138 and
correlates with the extended \tev source.  Therefore, We concluded that
these two sources were included in 2FGL to account for residual induced by
not taking the extension of this extended source into account and remvoed
them from our model of the sky.  There are two further away sources
...
of \tsext=16.3.  This source was fit to $(l,b)=(332\fdg38,-0\fdg14)$ and is coincident
with 1FGL\,J1613.6$-$5100c.
2FGL\,J1615.2$-$5138 was found
 to have an extension of $0\fdg42\pm0\fdg03_\stat\pm0.01_\sys$
with an extension significance of \tsext=48.0.  This source
was fit to $(l,b)=(331\fdg66,-0\fdg66)$ and is also coincident
with 1FGL\,J1614.7$-$5138c. To test for the possibility that
...
sources.  Because $\tsinc=37.0$ is less than \tsext, we conclue that
...
region of hte sky.  It is spatially coincident with the \tev source
...
2FGL J1634.4$-$4743c.4-4820c that correlate with the extended \tev source
but that do not separate from 2FGL\,J1632.4$-$4753c at \gev energies.
we concluded that these sources were included in 2FGL to account for
residual induced by not taking the extension of this extended source
into account and removed them from our model of the sky.
...
the extended \tev emission of HESS\,J1632$-$478 and that separate from
the extended source candidate 2FGL\,J1632.4$-$4753c at \gev energies.
Figure~\ref{1FGL_J1632.9-4802c}b shows the same region with these
background sources subtracted.

With this model of the sky, 2FGL\,J1632.4$-$4753c was found to have an
extension of $0\fdg44\pm0\fdg04_\stat\pm0\fdg03_\sys$ with an extension
significance of \tsext=64.5.  This source was fit to 
$(l,b)=(336\fdg41,0\fdg22)$ and is
also coincident with 1FGL\,J1632.9$-$4802c.  While fitting the extension
...
smoothed counts map of the region in and overlays the \tev contours of
...
sources which appears to correlate with the \tev morphology of
HESS\,J1837$-$069. Similarly, the emission from 2FGL\,J1835.5$-$0649
and 2FGL\,J1837.3$-$0700c does appear to separate at \gev energies.
Therefore, we concluded that this source was included in 2FGL to account
for residual induced by not taking the extension of this extended source
into account and removed them from our model of the sky.  We also removed
from our model of the sky the further away point-like catlaog soruce
...
region after subtracted the modeled background sources.


With our improved model, 2FGL\,J1837.3$-$0700c was found to have an
extension of $0\fdg35\pm0\fdg08_\stat\pm0\fdg03_\sys$ with an extension
significance of $\tsext=18.8$ using
 photons with energies between
10 \gev and 100 \gev.  This source was fit to $(l,b)=(25\fdg08,0\fdg13)$
and is also coincident with 1FGL\,J1837.5$-$0659c.  To get a good fit
of 2FGL\,J1837.3$-$0700c, we iteratively relocalized the three nearby
...
source confusion by fitting the source instead as two point sources.
Because $\tsinc=12.6$ is less than \tsext, we conclue that this emission
...
with 2CG\,078+01, 3EG\,J2020+4017, and 1FGL\,J2020.0+4049
\citep{second_cos_b_catalog,third_egret_catalog,first_cat} and also with
...
There is
a nearby source 2FGL\,J2019.1+4040 which correlates with the radio
...
residual induced by not taking the extension of $\gamma$-Cygni into
account and removed it from our model of the sky.  We also removed from
...
Furthermore, we chose to add to our region an additional point-like
source $(l,b)=(78\fdg85,2\fdg67)$.  Although this source is not very
significant ($\ts=13.5$ in the 1 \gev to 100 \gev energy range), its
location is outside the multiwavelenth contours of $\gamma$-Cygni and
modeling it improved the connection of the best fit extension
with the multiwavelenth size of $\gamma$-Cygni. Nevertheless,
the overall spatial and spectral characteristics of the source were not
significantly changed after including the source.

Using this sky model, 2FGL\,J2021.5+4026 fit an extension of
$0\fdg59\pm0\fdg03_\stat\pm0\fdg02_\sys$ with an extension significance
of $\tsext=116.4$.  This source was fit to $(l,b)=(78\fdg18,2\fdg19)$.
...
as can be seen in Figure~\ref{1FGL_J2020.0+4049}.
...
of the LAT data using the same elliptical surface brightness model.
...
\begin{figure}
    ...
    \caption{
    \newtext{
    The detection threshold to resolve a uniform surface brightness
...
\tablecomments{
      ...
      sources with a uniform surface brightness for a two-year exposure
...
\tablenotetext{(a)}{
Integral Flux is in units of $10^{-9}\phflux$ and integrated in the fit
...
\begin{figure}
  ...
  radially-symmetric uniform surface brightness.  The two 
...
\begin{figure}
    ...
    Vela X is from \cite{velax}.  Except for RX\,J1713.7$-$3946 and Vela Jr.,
    ...
    were not directly fit at \tev energies. Here, we take their sizes to be the
...
\begin{figure}
    ...
    to \rsixeight. Because
    the spatial morphology of RX\,J1713.7$-$3946 and Vela Jr. is poorly
    described by a 2D Gaussian surface brightness model, the \gev and \tev
    extensions are included assuming a uniform surface brightness model.
