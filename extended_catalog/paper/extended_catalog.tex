...
\todo[inline]{Ask Stefan/Markus if they think we should get rid of
this note about what point soruces in the plane.}

\todo[inline]{add note about how high energy emission may be hadron,
...
Center (\cite{fssc}).} and fit them them using \pointlike as both point
...
Puppis A's age was estimated to be $\approx3700$ years (\cite{winkler_1988}) and
its distance was estimated at 2.2 kpc (\cite{ reynoso_1995,reynoso_2003}).
Assuming this distance, the \gev luminosity of Puppis A between 1 and
100 \gev is $\approx 3\times 10^{34}$ ergs/\s.

Molecular clouds have not been detected ajacent to Puppis A (\cite{puppis_a_co})
Similarly, the luminsoity is lower than that of other LAT
detected SNRs interacting with molecular clouds (\cite{\dots}),.
The LAT emission could therefore be explained by the interaciton of acclerated
particles by interstellar matter or fields in the regions, similar
to what was suggested for the Cygnus Loop SNR (\cite{cygnus_loop_lat}).

Puppis A has not been observed to direct
% 1 - 100 GeV
%      Norm      
%      Index     
...
%            = 2.8e34 ergs/s from 1-100 gev


\todo[inline]{Make point about how this source
is like Cygnus Loop. Discuss with Yasunobu.  Main points about CO
Emission / moelcular cloud interactions (\cite{co_eastern_puppis_a}).
Can we say anything about emission could be hadronic or leptonic models?
Markus says to add SEDs. Should we?
}
...
(\cite{isabelle_dark_gass}). Embedded star-forming regions make it even
...
spatial correlation with the \gev emission (\cite{iras_rho_ophiuci}).
...
\todo[inline]{Emission due to SNR, PWN, Massive Star. Stefan
says to add Suzaku observations. Add note about SWIFT observations + citation.}
...
observed by INTEGRAL (\cite{Igr_J16320-4751_circ}), \xmmnewton
(\cite{xmm_newton_IGR_J16320-4751}), and \asca (\cite{asca_plane_survey}).
But it is suspected to be an galactic X-Ray Binary so the \gev and \tev
extension disfavors the association.

Further observations by \xmmnewton reveal extended emission of size
...
\begin{figure}
  ...
  \todo[inline]{Ask markus if this description of ophicucus is good}
...
\begin{figure}
  \begin{center}
    ...
    systematic only.
    \todo[inline]{Investigate quality of upper limits. Markus wants SEDs
    for other sources. Ask Stefan if he thinks we should add SEDs for
    more sources}
    }
    \label{hess_seds}
  \end{figure}
...
\begin{figure}
  \begin{center}
    ...
    2FGL\,J1615.0-5051, and 2FGL\,J1615.2-5138 (\cite{hess_plane_survey}).
    ...
    The \tev extension of IC443 is from (\cite{ic443_veritas})
    and W51c is from (\cite{w51c_with_magic_at_fermi_symposium}).
