...
LAT PSF, their PDF are similar for a source that has a 0\fdg5 radius
...
It leads to the often quoted result $\sqrt{TS}=\sigma$. It is plausible
...
for a source of a given flux and extension.
}
...
behavior of these plots is what one would expect. $\tsext-\tsinc$
is mostly positive when fitting the simulated extended sources in
Figure~\ref{confusion_extended_plot} whereas $\tsext-\tsinc$ is
mostly negative when fitting the two simulated point-like sources in
Figure~\ref{confusion_2pts_plot}.  
Furthermore, $\tsext-\tsinc$ becomes more positive when the extended
soruce becomes larger and more negative when the two point-like sources
are farther apart.
From this, we conclude that the $\tsext>\tsinc$ test is
powerful at discarding cases in which the true emission comes from
two point-like sources without frequently rejecting real extended sources.
...
spectral parameters) and can therefore easy model much of the extended
...
To improve our analysis of this source, removed
...
the spatial extension of HESS\,j1825$-$137 and MSH\,15$-$52 with the
LAT \citep{msh1552,fermi_hess_j1825}.
}
...
was found to have an extension $\sigma=0\fdg59\pm0\fdg03_\stat\pm0\fdg02_\sys$
with $\tsext=116.4$.  Figure~\ref{snr_seds}
...
\begin{figure}
    ...
    \caption{
    \newtext{
    Cumulative distribution of the test statistic for the extension test when fitting
...
\begin{figure}
    ...
    \caption{
    The projected detection threshold of the LAT to extension after ten years
    for a power-law
...
\begin{figure}
    ...
    (d) are the distribution of $\tsext-\tsinc$ for the same simualted sources.
...
\begin{figure}
    ...
    same spectral index and (d) and (e) represent simulations of two
...
\begin{figure}
    ...
    Gaussian fit and so the assumed size is the mean of the semi-major
    and semi-minor axis.
