...
we asses the bias that is introduced by assuming an incorrect spatial model.
...
\newtext{
We performed a second simulation to assess any bias that could
be introduces by performing the extension test in the presence of a highly
...
and sources much brighter are less likely to be affected by the background.
...
W44 are very significant ($\ts\sim3500$) when fit as point-like sources.
Figure~\ref{ts_comparison_w44sim}b shows that the simulations 
are highly extended ($\tsext\sim250$)
when fit with a radially-symmetric uniform disk or Gaussian spatial
model.
On average \tsext is
...
Figure~\ref{ts_comparison_w44sim}c shows that the sources were fit better
...
the sources were fit with extended spatial models.  When assuming
the elliptical ring spatial model, there is no significant bias in 
\rsixeight.  Because none of the other spatial models induced a
...
an alternative Bayesian formulation of this test, but it is difficult to apply
...
by not modeling the extensions of nearby extended sources.}
