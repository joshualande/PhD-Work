...
We performed a second set of simulation to show that the theoretical distribution
...
and the results for sources much brighter are less likely to be affected by the 
structured background.

Because the Galactic diffuse emission is highly structure with
...
We also performed a Monte Carlo study that (a) shows that we obtain
an unbiased fit of an extended source with a complicated spatial shape so long as
the source is correctly modeled and (b) quantifies the biases 
introduced by incorrectly modeling its spatial structure.
...
isotropic background model used in 2FGL.  
We simulated the gamma-ray emission in the 1 \gev to 100 \gev
energy range for the 2-year interval of the 2FGL catalog.
In the simulations, 
...
shows that
the significance of W44 in the simulations is very great ($\ts\sim3500$) 
for a model with a point-like source hypothesis.
Figure~\ref{ts_comparison_w44sim}b shows that 
the significance of the spatial extension is also very great
($\tsext\sim250$).  
...
Figure~\ref{bias_w44sim} shows that there are no systematic biases in the
distribution of best fit (a) flux (b) spectral index (c) \rsixeight
(d) elliptical eccentricity (e) elliptical orientation angle and (f) inner
ring fraction when the simulated extended sources were fit with the
elliptical ring spatial model. For the elliptical spatial models, \rsixeight
is computed by numeric integration.
Figures~\ref{bias_w44sim}a and~\ref{bias_w44sim}b show that
no significant bias is introduced by modeling the source with the 
other extended spatial models, while modeling the source as point-like
results in $\sim10\%$ and $\sim5\%$ biases in the fit flux and index,
respectively.  
Furthermore, Figure~\ref{bias_w44sim}c shows that the \rsixeight estimate of
the extension is very mildly biased ($\sim10\%$) toward higher values
when inaccurate spatial models are used, and thus \rsixeight for any of
these spatial models can be used as a reasonable measurement of the true
68\% containment radius for the source.
}
...
Because the two point-like source hypothesis has
...
Thus our criterion based on the \ts value is equivalent to requesting
that the \aic-based empirical support for the two-point-like-source model be
``considerably less'' than for the extended source model, following the
classification by \cite{aic_stats_book}.
}
...
\tablecomments{
\newtext{For each new extended source, we list nearby 2FGL
