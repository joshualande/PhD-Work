...
and sources much brigher are not likely to be affected by the background.
...
if the source was at the galactic center.  Table~\ref{ts_ext_num_sims}
...
of \tsext. For small values of \tsext,
...
\newtext{
We performed a Monte Carlo study to quantify the bias that is introduced by
incorrectly modeling the spatial structure of an extended source. 
...
The simulated flux and spectral index of this source
...
a semi-minor axis of 0\fdg19, has a position angle of $147\degree$
measured east of celestial north, and predicts emission only for the
outer 0.75\% of the shape.
...
Figure~\ref{ts_comparison_w44sim}a shows that the simulations of
W44 when fit as point-like sources are very significant ($\ts\sim3500$).
Figure~\ref{ts_comparison_w44sim}b shows that the simulations when
fit with a radially-symmetric uniform disk or Gaussian spatial
model are highly extended ($\tsext\sim250$).  On average \tsext is
slightly larger when fitting the sources with a uniform disk spatial
model compared to a Gaussian spatial model, consistent with the
fact that the simulated source's true spatial model has a sharp edge.
Figure~\ref{ts_comparison_w44sim}c shows that the sources fit better
...
Finally, Figure~\ref{ts_comparison_w44sim}d shows that the sources
are only fit slightly better assuming an elliptical ring spatial model
compared to an elliptical disk spatial model ($\ts_\text{elliptical\
ring}-\ts_\text{elliptical\ disk}\sim9$).

Figure~\ref{bias_w44sim}a and Figure~\ref{bias_w44sim}b
show the distributions of best fit fluxes and spectral indices when fitting
...
(within the precision of the Monte Carlo study). This is because,
within the finite angular resolution and limited statistics of the LAT, 
all of the extended spatial models can be adjusted to look very similar.

Figure~\ref{bias_w44sim}c plots the best fit 68\% containment radius when
the sources are fit with the extended spatial models.  When assuming
the elliptical ring spatial model, there is no significant bias in the
best fit size.  Becasue none of the other spectral models induced a
bias greater than $\sim10\%$, we see that \rsixeight for any of the
spatial models can be used as a reasonable measure of the true 68\%
containment radius for the source.
...
inner ring fractions is unbiased when the sources are fit with the elliptical
ring spatial model.
}
...
an alternate Bayesian formulation of this test. But it is difficult to apply
to LAT data because it contains a term including the number of data points. But for LAT data,
there are many photons included in an analysis that contribute only
 marginally to the analysis of a source
...
we note that the $\tsext>\tsinc$ test is mathematically equivalent to testing 
\begin{equation}
  \aic_\text{ext} < \aic_\text{2pts} - 6.
\end{equation}
Therefore, our test has an 
...
and does not show separated emission in the observed photon distribution.}
...
the flux for sources placed 
in the Galactic center. The actual flux is scaled by Equation~\ref{scale_flux_by_background}.}
}
