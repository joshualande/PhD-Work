...
\newtext{In section XXX, we perform a dedicated monte carlo simulation
...
\newtext{
These previous simulations were preformed assuming an isotropic backgorund.
We performed a second test only 

The flux of the source was scaled as
\begin{equation}
  F(\vec{x}) = F(GC) \times \left(\frac{B(\vec{x})}{B(GC)
\end{equation}

.
The benifitted from probing a large range of fluxes and spectral indices
and by testing two energy ranges.
But were unable to assess any possible systematics that may be due to
structured diffuse emission. 

Assessing possible difficulties due to having
an incorrect

We performed an additional Monte Carlo test with sources

Figure~\ref{tsext_plane_plot}

We note that this study does not asses issues due to incorrectly modeling the 
galactic diffuse background which are much more difficult to assess. But
this test does show the power of our tools when the model is good (or something).
...
and summarized in Table~\label{known_extended_sources}. In particualr W44 was simulated
...
top of the standard galactic and isotropic diffuse background model used
in 2FGL.  This simulation was in the 1 \gev to 100 \gev energy range and
