...
we asses the bias that is is introduced by using an incorrect spatial model.
...
\newtext{
These previous simulations spaned the range of detectable fluxes 
over two different energy ranges, but only assuming an isotropic backgorund.
...
Our simulation setup was the same as above except that the sources
were simulated on top of and analytized assuming the presence of
the standard Galactic and isotropic diffuse background model used
in 2FGL.

In our simulation, we selected our sources to have a random position on the sky
such that the absolute value of the galactic latitude of the source is
less than 5\degree, probing the most structured areas of the galaxy.
...
The source flux was selected so that $\ts\sim50$. This is around 
the significane level for which on might
exepcted to be able to spatially resolve the source. 
We do not test brigher sources since the structured background
should produce the most significant effect for very bright sources.

Because the Galactic diffuse emission is highly structure, the point-source
sensitivity can vary signficantly across the galactic plane.
To account for this, we scaled the flux (for a given spectral index)
by the overal background level according to this formula:
\begin{equation}
  \label{scale_flux_by_background}
  F(\vec{x}) = F(\text{GC}) \times \left(
  \frac{B(\text{GC})}{B(\vec{x})}\right)^{1/2}.
\end{equation}
Here, $\vec{x}$ is the position of the source, $F$ is the integral
flux of the simulated source from 100 \mev to 100 \gev, $F(\text{GC})$
is the same quantity if the source were at the galactic center, $B$
is the integral of the Galactic diffuse and isotropic diffuse emission
model from 1 \gev to 100 \gev and $B(\text{GC})$ is the same quanitty
if the source were at the galactic center.  Table~\ref{ts_ext_num_sims}
tabulates $F(\text{GC})$ and the average \ts for the four spectral models.
For each spectral index, we performed $\sim90,000$ simulations.
...
there is reasonable agreement between the Monte Carlo simulation and
theory.  At the highest obtained values of \tsext, there is possibly
some discrepancy.  But in this range there are only limited statistics
because the curve is determined by only a few points.


}

\subsection{\newtext{Assessing the Bias Due to Incorrect Spatial Models}}
\label{bias_wrong_spatial_model}

\newtext{
We performed a Monte Carlo study to asses the biases introudced by incorreclty
modeling the spatial structure of an extended source. This is important because any assumed spatial
model is just an approximation to the true photon distribution of the source.

To assess this systematic, we simulated the spatially extended ring-type SNR W44.
W44 was slelected because it is the most signfiicant extended source detected by the LAT 
that has been reported as having a non-radially symmetric photon-distribution \cite{w44}. 

W44 was simulated with a power-law spectral model. The flux and spectral index of this source
were taken from our re-analysis in this paper which is described in Section~\ref{validate_known}
and summarized in Table~\ref{known_extended_sources}. In particualr W44 was simulated
with a flux of $7.12\times10^{-8}$ \phflux in the energy range from 1 \gev to 100 \gev with a spectral
index of 2.66.

W44 was simulated with an elliptical ring spatial model described in
\cite{w44}.  Because that publication does not provide the numbers describing
the parameters of the ellipse, we summarize them here for completeness. The elliptical
spatial model has a major axis of 0\fdg3 and a minor axis of 0\fdg19. The
ellipse predicts emission only for the outer 0.75\% of the ellipse and
the ellipse has an angle of $147\degree$ measured east of celestial north.

We used a simulation setup similar to that described in
Section~\ref{monte_carlo_validation}, but the souce was simulated on
top of and analyzed assuming the presence of the standard galactic and
isotropic diffuse background model used in 2FGL.  This simulation was
in the 1 \gev to 100 \gev energy range and
the sources were simulated over the two-year time range included in the
2FGL catalog. We performed 985 identical simulations with different seeds to
see the statistical spread in the fit values.

After simulating the ring-like source, we fit the spectral and spatial parametesr of 
the ring-like extended source with successivly more complicated spatial models.
We obtained the best fit spatial parameters using \pointlike and, using 
these best fit spatial paramters, obtained the best fit spectral paramters 
using \gtlike. In total, we performed the fit using a point-like spatial model,
using a radially-symmetric uniform Gaussian and disk spatial model, 
an Elliptical disk spatial model, and finally the Monte Carlo true Elliptical
ring spatial model.


Figure~\ref{ts_comparison_w44sim} shows the the distribution of TS values for 
each of the simluations when fitting the simulated sources 
with the different spatial models. 
Figure~\ref{ts_comparison_w44sim}a shows the distribution of \ts values when 
fitting the simulated Elliptical ring extended source as as a point-like source.
the source is very significant as a point source ($\ts\sim3500$).
Figure~\ref{ts_comparison_w44sim}b shows the distirbution of \tsext values
when fitting the source as a radially-symmetric uniform disk or Gaussian source.
The source is very significiantly extended ($\tsext\sim250$). 
...
Considering that the simulated source's true spatial model has a sharp edge, it
makes sense that the Disk would provide a better fit.

Figure~\ref{ts_comparison_w44sim}c then shows the change in \ts (or
twice the improvement in log likelihood) when fitting the simulated source as
an elliptical disk compared to fitting the source as a disk. Since the change
in \ts is $\sim30$, we see that our simulated source is significantly better
described with an ellpitical spatial model. Finally, 
Figure~\ref{ts_comparison_w44sim}d shows the change in \ts when fitting
the source as an elliptical ring compared to fitting it as an elliptical disk.
Since the change is $\sim9$, we see that the source is somewhat better described
by the true spatial model, but not at a high statistical significance.


Figure~\ref{bias_w44sim} shows the distirbution of best fit parameters
for these simulations. Figure~\ref{bias_w44sim}a and Figure~\ref{bias_w44sim}b
show the distribututions of best fit flux and spectral indices.
They show that when fitting this simulated extended source with a point-like
spatial model, there is a $\sim$10\% bias in the fit flux and
$\sim5\%$ bias in the fit index. But when fitting the sources with any of
the extended spatial models, there is no bias in the best fit spectral parameters
(within the precision of this Monte Carlo study. The reason for this is
because within the finite angular precision and limited statsitics of the LAT, 
all of the extended spatial models can be ajusted to look almost the same.
For brighter or larger sources, this could potentially be a larger effect.

Figure~\ref{bias_w44sim}c plots the best fit 68\% contaiment radius
(\rsixeight).  For the elliptical ring spatial model, as expected
there is no significant bias in the best fit size. What is surprising
is that for none of the tested spatial models was the bias greater
than $\sim10\%$. This shows that for whatever spatial model is used,
the best fit \rsixeight is a reasonable measure of the true 68\%
containment radius.

For completeness, for the elliptical disk and ring spatial models,
Figure~\ref{bias_w44sim}c plots the distribution of best fit eccentricity
of the ellipse. It is defined as $\sqrt{1-b^2/a^2}$ where $a$ is the
length of the semi-major axis and $b$ is the length of the semi-minor
axis.  Figure~\ref{bias_w44sim}c plots the distribution of best fit
orientation angle of the ellipse. For the elliptical spatial models,
there is no systematic bias in the eccentricities or orentiation angles.
For the true sptial model, there is less spread in the distributions of
the fit parameters.  Finally, for completeness Figure~\ref{bias_w44sim}d
shows that the distirubtion of best fit ring fractions is unbiased when
the source is fit with the for the elliptical ring spatial model.
...
\newtext{
It might be desirable to formulate a Bayesian test comparing an extended
source to two point-like sources.
...
and for LAT events, there are many photons included in the ROI that contribute only
 marginally when considering different models.
We also considered the Akaike information criterion \citep[\aic,][]{AIC_statistical_test}.
The \aic is defines as
\begin{equation}
  \aic=2k-2\log\likelihood
\end{equation}
where $k$ is the number of parameters in the model. 
The first term in the \aic penaize models with additonal parameters. 
In this formulation, the best model is consided to be the one that minimzies the \aic.
Because the two point-like source hypotheses has
three more parameters compared to the one extended source hypothesis (two spatial
parameters and two spectral parameters compared to one extnesion parameter),
we note that the $\tsext>\tsinc$ is mathematically equivalent to the test that
\begin{equation}
  \aic_\text{ext} < \aic_\text{2pts} - 6,
\end{equation}
which gives our original test an equivalent Bayesian formulation by requring 
$\aic_\text{ext}$ to be sufficently smaller than $\aic_\text{2pts}$.
}
...
by not modeling the extension of a nearby source.}
...
\hline
      ...
      \hline                                                
        3 &          $10^{-5}$ &           18354 &  19466    &     18354   &     215    \\
          ...
          &          $10^{-7}$ &           13714 &     41    &   \nodata   & \nodata    \\
\cutinhead{Galactic and Isotropic Diffuse Background\tablenotemark{(b)}}
      1.5 &  $3\times 10^{-7}$ &           90741 &     63    &   \nodata   & \nodata     \\
        2 &          $10^{-6}$ &           92161 &     60    &   \nodata   & \nodata     \\
      2.5 &  $3\times 10^{-6}$ &           86226 &     47    &   \nodata   & \nodata    \\
        3 &          $10^{-5}$ &           94412 &     61    &   \nodata   & \nodata    \\
\enddata
...
if it was placed in the galactic center and is scaled by Equation~\ref{scale_flux_by_background}.}
}
...
\tablecomments{
    ...
    the bottom rows are the simulations on top of the Galactic and isotropic
    background.}
}
\end{deluxetable}
...
\begin{figure}
    ...
    simulated on top of the Galactic diffuse and isotropic diffuse
    emission.
    }
...
\begin{figure}
    ...
    \caption{
    \newtext{The distirbution of \ts values when fitting statistically
    independent simulations of the
    elliptical ring shaped SNR W44. (a) is the distribution of \ts values
    when fitting W44 as a point-like source and (b) is the
    distribution of \tsext when fitting the SNR with a Gaussian and uniform disk 
    ...
    fitting it with a radially symmetric spatial model and (d) 
...
\begin{figure}
    ...
    The plots show the distribution of (a)
...
\startdata
\hline
2FGL\,J0823.0$-$4246    & 2FGL\,J0823.4$-$4305, 2FGL\,J0821.0$-$4254 \\
2FGL\,J1627.0$-$2425c   & \nodata \\
2FGL\,J0851.7$-$4635    & 2FGL\,J0853.5$-$4711, 2FGL\,J0848.5$-$4535, 2FGL\,J0855.4$-$4625 \\
2FGL\,J1615.0$-$5051    & \nodata \\
...
\tablecomments{
\newtext{
For each of the new extended sources, we list any catalog
source that was included in 2FGL to account for residuals induced
by not modeling the extension of a nearby source.
These sources were removed from our model of the sky when studying
the extended source.
}
}
\end{deluxetable}
...
\begin{figure}
  ...
  \newtext{From left to right, these sources are, from left to right,
  2FGL\,J0823.4$-$4305 and 2FGL\,J0821.0$-$4254.}
