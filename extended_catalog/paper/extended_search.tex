...
\newtext{
These simulations spanned the range of detectable fluxes 
over two different energy ranges, but only assuming an isotropic background.
...
were simulated on top of and analyzed assuming the presence of
...
We do not test brighter sources since the structured background
...
so that the source always has approximatly the same signal-to-noise ratio:
\begin{equation}
...
model from 1 \gev to 100 \gev and $B(\text{GC})$ is the same quantity
...
\newtext{
We performed a Monte Carlo study to asses the biases introduced by incorrectly
...
W44 was selected because it is the most significant extended source detected by the LAT 
...
and summarized in Table~\ref{known_extended_sources}. In particular W44 was simulated
...
In the simulations of W44, we disabled the inherent energy dispersion of photons
...
After simulating the ring-like source, we fit the spectral and spatial parameters of 
the ring-like extended source with successively more complicated spatial models.
...
Figure~\ref{ts_comparison_w44sim} shows the distribution of TS values for 
each of the simulations when fitting the simulated sources 
...
because within the finite angular precision and limited statistics of the LAT, 
all of the extended spatial models can be adjusted to look almost the same.
