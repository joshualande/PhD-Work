...
if args.model != "None":
    ...
    print args.model
    print name
#    name2=name[0:3]+'_'+name[3:len(name)]
#    print name2
    for srcmodel in glob.iglob("%s/%s*.xml"%(args.model,name)):
        print srcmodel
        modelloc=srcmodel
        maxroi=10.0
...
def renorm(roi,fact,sens="init"):
    """Function to renormalize the source model
    sens = "init" : initialization of the roi -> * phasefactor
    sense = "revers" : analysis ended -> come back to all phases values"""
    if sens=="init":
        for names in roi.get_names():
            try :
                roi.modify(names,Norm=roi.get_model(names)[0]*fact)
            except :
                try :
                    roi.modify(names,Int_flux=roi.get_model(names)[0]*fact)
                except :
                    try :
                        roi.modify(names,Norm=roi.get_model(names)[0]*fact)
                    except :
...
def renorm_gtlike(like,fact,sens="init"):
     """Function to renormalize the source model
     sens = "init" : initialization of the roi -> * phasefactor
     sense = "revers" : analysis ended -> come back to all phases values"""
     if sens=="init":
         for names in like.sourceNames():
             funcs = like[names].src.getSrcFuncs()
             try :
                 funcs['Spectrum'].getParam('Prefactor').setValue(funcs['Spectrum'].getParam('Prefactor').getValue()*fact)
             except :
                 try :
                     funcs['Spectrum'].getParam('Integral').setValue(funcs['Spectrum'].getParam('Integral').getValue()*fact)
                 except :
                     try :
                         funcs['Spectrum'].getParam('Norm').setValue(funcs['Spectrum'].getParam('Norm').getValue()*fact)
                     except :
                         print names
     else :
         for names in like.sourceNames():
...
fact=roi.phase_factor
renorm(roi,fact,sens="invers")
print roi
...
renorm(roi,fact,sens="init")
roi.toXML(filename="srcmodel_prefit_renorm_%s.xml"%name,convert_extended=True)
...
r['TS_pointlike']=TS_pointlike

renorm(roi,fact,sens="revers")

roi.toXML(filename="srcmodel_output_pointlike_%s.xml"%name,convert_extended=True)
...
for namec in like.sourceNames():
    if namec.find('eg_')==-1 and namec.find('gal')==-1:
        try :
            gtlike.like[gtlike.like.par_index(namec, 'Prefactor')].setBounds(0.0,1.0e7)
        except :
            gtlike.like[gtlike.like.par_index(namec, 'Integral')].setBounds(0.0,1.0e7)
...
#outputModel = "output_model_gtlike_%s.xml" %(name)
...
outputModel = "output_model_gtlike_renorm_%s.xml" %(name)
like.srcModel = outputModel
like.writeXml()
...
TS_gtlike=like.Ts(name,reoptimize=True)
    
print "TS_gtlike=%.2f"%TS_gtlike

r['TS_gtlike']=TS_gtlike
    


renorm_gtlike(like,fact,sens="revers")
...
funcs = like["gal_v02"].src.getSrcFuncs()
...
roi.plot_tsmap(filename='residual_tsmap_0.1_%s.png' % name,fitsfile='residual_tsmap_0.1_%s.fits'% name,pixelsize=0.10,size=10)
roi.plot_tsmap(filename='residual_tsmap_0.25_%s.png' % name,fitsfile='residual_tsmap_0.25_%s.fits'% name,pixelsize=0.25,size=10)

roi.toRegion('Region_file_%s.reg'%name)

roi.plot_sources(filename='sources_%s.png' % name, size=10, label_psf=False)
...
roi.plot_tsmap(filename='source_tsmap_0.10%s.fits' % name, fitsfile='source_tsmap_0.1_%s.fits'% name,pixelsize=0.10,size=10)
...
roi.plot_tsmap(filename='source_tsmap_0.25%s.png' % name,fitsfile='source_tsmap_0.25_%s.fits'% name,pixelsize=0.25, size=10)
roi.unzero_source(which=name)
