...
def setup_tev(name, tevsources, ft1,ft2,ltcube,fit_emin, fit_emax, extended=False, **kwargs):
    ...
    binsperdec=4
    binfile=e('/afs/slac/g/glast/users/rousseau/TeV_sources/bexpmap/binned_%sbpd_%s_hard_source.fits' %(binsperdec,name))
    ...
    if isnum(ext):
        ...
        raise Exception("Unrecogized size of sigma" % sigma)
    ...
    for source in roi.get_sources():
        try :
            source2=catalog1.get_source(source.name)
            try :
                roi.del_source(source.name)
                roi.add_source(source2)
            except :
                print "la source existe mais je suis incapable de la changer"
        except :
            print "la source n'existe pas"
...
def get_catalog(**kwargs):
    return FermiCatalog("/afs/slac/g/glast/users/rousseau/catalogs/hard_source_list/gll_psc3yearhardclean_v2r1.fit")
